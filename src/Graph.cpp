...
// Impovement strat: make a heap with the sucessors of each v 
// such that the root is the sucessor with highest distance relative to s
// find all the maximum cycles in s in one dfs
...
std::vector<std::vector<int>> Graph::findBlocksByCycle()
{
    ...
    for (int v = start; v < n; v++) {
        ...
        bfs(v, -1, level);
        findMaxCycles(v, level, new_cycles);
        bool anexed = false;
...
#if 0
    bool founded = false, isNewCycle = true;
    for (int v = start; v < n; v++) {
...
void Graph::findMaxCycles(int s, std::vector<int>& level, 
std::vector<std::unordered_set<int>>& cycles)
{
    std::vector<int> clojure, visitCount(n, 0);
